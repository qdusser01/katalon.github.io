...
var scHPS = {
	...
	init : function() {
		...
		} catch(e){scCoLib.log("ERROR scHPS.init : " + e);}
	},
	...
	onLoad : function() {
		...
		}catch(e){scCoLib.log("ERROR PresMgr.onLoad : " + e);}
	},
	loadSortKey : "AA",
	 ...
	 * @param pRate 2 valeurs possibles: 0 (invisible) ou 1 (visible). */
	xEndOpacityEffect: function(pNode, pRate, pIsDelay){			
		if(!this.fNavie8) pNode.style.opacity = pRate;
		...
		} else if(pRate == 0) pNode.style.visibility = "hidden";			
...
scHPS.FadeEltTask = function(pElt,pDir,pMgr,pStartFunc,pEndFunc,pInstant){
	...
	}catch(e){scCoLib.log("ERROR scHPS.FadeEltTask : "+e);}
}
scHPS.FadeEltTask.prototype = {
	...
	execTask : function(pInstant){
		...
		if(this.fIdx >= this.fRate[this.fDir].length) {
			scHPS.xEndOpacityEffect(this.fElt, this.fDir, this.fDir == 0 ? this.fMgr.fEnableCssEffects : false);
			this.fEndFunc();
...
scHPS.PresMgr.prototype = {
		...
		try{
			...
			for (var i=0;i<vLocalSlds.length;i++){
				...
				for (var j=0;j<this.fSldRules.length;j++){
					...
					} catch(e){scCoLib.log("WARNING PresMgr.onLoad - local slide rule num. " + j + ": "+e);}
				}
	...
	/** PresMgr.enableCssEffects : Enable / Disable visual css effects */
	enableCssEffects : function(pEnable) {
		if (scHPS.fDisabled) return;
		this.enableEffects(false);
		this.fEnableCssEffects = pEnable;
	},
	...
	xGotoSlide : function(pIdx, pFromStart, pInstantResult) {
		try{
			...
			return true;
		}catch(e){scCoLib.log("ERROR PresMgr.xGotoSlide : "+e);}
	},
	...
	xSetCurrSlide : function(pNewCurrSlide) {
		...
		}catch(e){scCoLib.log("ERROR PresMgr.xSetCurrSlide : "+e);}
	},
	...
	xNotifyListeners : function(pType,pRes) {
		...
		for (var i=0;i<vListener.length;i++) {
			try {
				vListener[i](pRes);
			} catch(e) {scCoLib.log("ERROR PresMgr.xNotifyListeners : "+e);}
		}
	},
		...
		if (!this.fToolsHidden) {
			...
			if (vNewTime-this.fToolsShowTime >= 5000) {
				...
				for (var i=0;i<this.fToolElts.length;i++) {
					...
					} catch(e) {scCoLib.log("ERROR PresMgr.xHideTools : "+e);}
				}
			}
		}
	},
	...
	xShowTools : function(){
		if (this.fToolsHidden) {
			...
			for (var i=0;i<this.fToolElts.length;i++) {
				...
				} catch(e) {scCoLib.log("ERROR PresMgr.xShowTools : "+e);}
			}
		}
		this.fToolsShowTime = new Date().getTime();
	},
	...
	fSwitchSldTask : {
		...
		initTask : function(pNewSld, pStatus, pInstantResult){
			...
			}catch(e){scCoLib.log("ERROR PresMgr.fSwitchSldTask.initTask : "+e);}
		},
		...
		execTask : function(){
			...
			}catch(e){scCoLib.log("ERROR PresMgr.fSwitchSldTask.execTask : "+e);}
		},
		...
		precipitateEndTask : function(){
			...
			if(this.fPresMgr.fCurrSld) scHPS.xEndOpacityEffect(this.fPresMgr.fCurrSld, 0, this.fPresMgr.fEnableCssEffects);
			if(this.fStatus != "") {
			...
			this.fIdx = -1;
			this.fStatus = null;
			}catch(e){scCoLib.log("ERROR PresMgr.fSwitchSldTask.precipitateEndTask : "+e);}
		},
		...
		rollbackTask : function(){
			...
			this.fIdx = -1;
			this.fStatus = null;
			}catch(e){scCoLib.log("ERROR PresMgr.fSwitchSldTask.rollbackTask : "+e);}
		}
	},
	...
	fSwitchBlkTask : {
		...
		initTask : function(pOldBlock, pNewBlock, pTarget, pInstantResult, pStartFunc, pEndFunc){
			...
			}catch(e){scCoLib.log("ERROR PresMgr.fSwitchBlkTask.initTask : "+e);}
		},
		...
		execTask : function(){
			...
			}catch(e){scCoLib.log("ERROR PresMgr.fSwitchBlkTask.execTask : "+e);}
		},
		/** PresMgr.fSwitchBlkTask.precipitateEndTask : Précipite la fin de la task en cours. */
		precipitateEndTask : function(){
			try{
			if(!this.fNewBlock) return;
			if(this.fOldBlock) scHPS.xEndOpacityEffect(this.fOldBlock, 0, this.fPresMgr.fEnableCssEffects);
			...
			}catch(e){scCoLib.log("ERROR PresMgr.fSwitchBlkTask.precipitateEndTask : "+e);}
		}
	},
	...
	fSwitchStpTask : {
		...
		initTask : function(pBlock, pTargetStep, pInstantResult){
			try{
				...
				return true;
			}catch(e){scCoLib.log("ERROR PresMgr.fSwitchStpTask.initTask : "+e);}
		},
		...
		execTask : function(){
			...
			}catch(e){scCoLib.log("ERROR PresMgr.fSwitchStpTask.execTask : "+e);}
		},
		/** PresMgr.fSwitchStpTask.precipitateEndTask : Permet de précipiter la fin de la task en cours. */
		precipitateEndTask : function(){
			try{
			if(! this.fBlkHdr) return;
			if (this.fPresMgr.fEnableCssEffects && this.fBlkHdr.fCurrStep >= 0) this.fBlkHdr.fBlkContent.classList.add("animate");
			//On place le content
			...
			if (this.fPresMgr.fEnableCssEffects) {
				scHPS.xSetOpacity(this.fNewMask, 0);
				var vNewMask = this.fNewMask;
				setTimeout(function(){
			...
			this.fOldMask = null;
			this.fNewMask = null;
			}catch(e){scCoLib.log("ERROR PresMgr.fSwitchStpTask.precipitateEndTask : "+e);}
		}
	},
	/* --- Utilities ---------------------------------------------------------- */
	/** PresMgr.xAddBtn */
...
scHPS.SldMgr = function(pRootPath, pWin, pSldHdr){
	...
	}catch(e){scCoLib.log("ERROR SldMgr : "+e);}
}
scHPS.SldMgr.prototype = {
	...
	onLoad : function(){
		...
		}catch(e){scCoLib.log("ERROR SldMgr.onLoad : "+e);}
	},
	/** SldMgr.getBlksRoot */
...
scHPS.SldLocalMgr = function(pRootNode,pSldHdr){
	try{
		...
		this.fBlksPath = scHPS.fBlksPath;
		this.fDefaultAnimStep = scHPS.fDefaultAnimStep;
	}catch(e){scCoLib.log("ERROR SldLocalMgr : "+e);}
}
...
scHPS.SldHdr.sOnloadSlide = function(){
	try{
		...
		vSldHdr.postOnLoad();
	} catch(e){scCoLib.log("ERROR SldHdr.sOnloadSlide : "+e);}
}
...
scHPS.FraZmHdr.sOnLoad = function() {
	...
	} catch(e){scCoLib.log("ERROR FraZmHdr.sOnLoad : "+e);}
}
	...
	xPlanSteps : function(){
		...
		function score(pNode){
			try{
				...
				return 1000000 + vH;
			}catch(e){scCoLib.log("WARNING : BlkHdr.xPlanSteps.score : "+e);}
		}
		do {
			var vBestScore = -1;
			var vBestIdx = -1;
...
scHPS.BlkCoHdr = function(pParentHdr, pBlkCoNode, pBlkIdx){
	...
	this.fSldMgr.fSiLib.addRule(pBlkCoNode, this);
}
scHPS.BlkCoHdr.prototype = {
	...
	xFixContainerSize : function(){
		var vAvailH = this.fParentHdr.getAvailHeight();
		var vDeltaHeight = (this.fBlkNode.offsetHeight - this.fBlksRoot.offsetHeight - this.fBlksRoot.offsetTop)/2;
...
scHPS.AnimBlk.prototype = {
	...
	onResizedAnc : function(pOwnerNode, pEvent){
		if(pEvent.phase==1) {
			...
			}catch(e){scCoLib.log("ERROR AnimBlk.onResizedAnc : "+e);}
		}
	},
	...
	execTask : function(){
		...
		}catch(e){scCoLib.log("ERROR AnimBlk.execTask : "+e);}
		return true;
	}
}
/** AnimBlk.sBtnPrv. */
...
scHPS.SizeBlkImg = function(pElt, pOpt){
	...
	if(!vImg) {
		scCoLib.log("SizeBlkImg - WARNING : cannot find base image element.");
		return;
	}
	pElt.fImg = vImg;
...
scHPS.SizeBlkImg.prototype.onResizedAnc = function(pOwnerNode, pEvent){
	if(pEvent.phase==1) {
		...
		}catch(e){scCoLib.log("ERROR SizeBlkImg.onResizedAnc : "+e);}
	}
}
...
scHPS.SizeBlkObj = function(pElt, pOpt){
	...
	if(!vObj) {
		scCoLib.log("SizeBlkObj - WARNING : cannot find base object element.");
		return;
	}
	pElt.fObj = vObj;
	pElt.fResFra = scPaLib.findNode(this.fPathResFra, pElt);
...
scHPS.SizeBlkObj.prototype.onResizedAnc = function(pOwnerNode, pEvent){
	if(pEvent.phase==1) {
		try{
			...
			if(!vObj) return;
			if(!vObj.oriW) vObj.oriW = vObj.width || vObj.clientWidth || 500;
			if(!vObj.oriH) vObj.oriH = vObj.height || vObj.clientHeight || 400;
			var vIsAlone = ! this.isEltNotAlone(pOwnerNode.fSldMgr.findOwnerBlk(pOwnerNode), pOwnerNode);
			var vRate = pOwnerNode.fSldMgr.getRatioNormalScreen() * pOwnerNode.fRatio;
		...
		}catch(e){scCoLib.log("ERROR SizeBlkObj.onResizedAnc : "+e);}
	}
}
